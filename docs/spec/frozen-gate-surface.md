...
- **Additive top-level fields embedded by the orchestrator** (set after `evaluate_gate` returns; consumers must tolerate their presence):
  ...
  - `fail_closed_triggered: bool` — **CONDITIONAL** addition. Present only when the caller supplies `fail_closed=True` to `run_production_gate` AND at least one collector outcome reports `status in {"error", "timeout"}` (i.e. `_collect_error_evidence` returns a non-empty label set). Always literally `True` when present (the key is the signal; presence alone implies an operator-visible override fired). Stamped on BOTH the fresh-gate path (`gate_orchestrator.py:1255`) and the reuse path (`gate_orchestrator.py:1085`) so the operator-facing audit trail is consistent across modes. Pinned by phase-2 + the reuse-path `fail_closed` follow-up; consumers MUST tolerate this key on every `run_production_gate` return.
  - `fail_closed_categories: list[str]` — **CONDITIONAL** addition. Present only when `fail_closed_triggered` is also present (same trigger condition). Carries the sorted set of distinct `category` labels whose collector outcomes triggered the override; sort order is ASCII-ascending so reruns are deterministic. Stamped on the fresh path at `gate_orchestrator.py:1256` and on the reuse path at `gate_orchestrator.py:1086`. The list is the operator's audit trail for "which categories sank the gate"; renaming or dropping it breaks every dashboard consuming `_bmad/gate/verdicts/<gate_id>.json` plus the `tests/test_orchestrator_fail_closed.py` regression net.
...
- **Soft-limit waiver:** `core/gate_orchestrator.py` historical baseline was 746 LOC pre-B / 834 LOC post-B; current LOC is **1409** at HEAD (against the 500-LOC soft limit). The original +88 LOC post-B delta covered B1's two-sided bound (with worked-example comments), B2's two `try / acquire / finally release` wraps around the `get_gate_lock` call sites in `recover_from_crash` and `run_production_gate`, and the supporting imports + module constants. The subsequent +466 LOC of post-B growth breaks down as: K-5 quarantine-under-lock + janitor (+90), K-2 evidence-bundle memoization (+5), C2 `lineage_root` embed (+8), C1 `drift_watcher` kwarg + double-poll wiring (+30), C3 `session_usage` kwarg + `cost_total_usd` stamp (+35), C5 `threshold_proposer` kwarg + audit event (+44), G2 `isolation_mode` + `max_workers` per-unit fan-out (+31), +60 LOC across three earlier gate-correctness follow-ups (additive-field-on-reuse fix, `_pending_cleanup` drain on every exit path, and quarantine-on-empty-`gate_id`), +90 LOC across five later gate-correctness follow-ups (evidence-cache invalidation on recovery rename, `fail_closed` override on reuse path, `fail_closed` timeout-audit inclusion, finally-OSError swallow to preserve `KeyboardInterrupt`, drift-watcher early-return docstring clarification), and +73 LOC for the round-2 recovery-descriptor surfacing fix (additive `recovery: {...}` subdict on `run_production_gate`'s return when mid-startup `_recover_from_crash_locked` quarantined a corrupted marker OR reported `cleanup_failed`, closing the §9.2 "loud, not silent" gap at the orchestrator's single integration point). The further +109 LOC growth from 1300 → 1409 covers two round-3 gate-orchestrator BaseException-safety fixes (`clear marker on drift_watcher BaseException` and `drain pending_cleanup on clear_gate_marker OSError`) plus their inline justification comments. The B2 observability helpers remain extracted into `core/gate_lock_observability.py` (~145 LOC). The next *broad* refactor that touches `gate_orchestrator.py` is expected to split adjudication/lifecycle into sibling modules (target ≤ 500 LOC); the +900 LOC overrun on the soft limit makes that split increasingly urgent. **The current LOC value above is a point-in-time snapshot; the binding contract is the symbol surface in this doc, not the LOC count. Bump the cited number whenever the live file drifts by more than ±50 LOC; the regression test in `tests/test_docs_consistency.py` pins a 150-LOC tolerance band as a hard ceiling.**

### `core/action_enum.py` (Path B / N6.6)
- `Literal` type for verifier actions consumed by `route_gate_verdict` and `production_ready_gate`; closed vocabulary `{"done", "remediate", "park", "defer", "escalate"}` (matches `VALID_ACTIONS` in `core/action_enum.py`). Adding a value requires a coordinated change in route + verifier + telemetry.
