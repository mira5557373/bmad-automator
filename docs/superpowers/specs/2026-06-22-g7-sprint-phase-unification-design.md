...
| Concurrent writers | The lock serializes writers — but readers do **not** take the lock by default (read-mostly contention model). **Read-order is the REVERSE of write-order** (gap D05): writer commits *phase store first*, *sprint-status second*; therefore the reader reads **sprint-status first** and *phase store second*. This pairing guarantees that a reader observing the new sprint-status also sees the new phase store (the matching write was committed first). The reverse coupling is pinned in the public docstring + an explicit code comment. **Two-read consistency**: the reader uses a stat-twice-or-retry pattern (gap D04) — after reading sprint-status + phase, re-stat both files; if either mtime is newer than the at-read-start stat, restart the read (capped at 3 attempts to bound retry storms; after 3, acquire the lock briefly to take a serialised snapshot). This eliminates the "two reads → writer between → phantom inconsistency → reader writes wrong projection" race. |
| Read-only callers (forensic / audit) | **`observe_only=True` kwarg ships in G7** (gap D07 — NOT deferred). `read_unified_state(root, key, *, observe_only=False)` defaults to the current write-on-repair behavior; when `observe_only=True`, the function never writes (no migration, no LWW repair), instead returning a 3-tuple `(sprint_status, phase, needs_repair: bool)` where `needs_repair` flags that the in-memory result is from a divergent on-disk state. Forensic tools (`tar -czf snapshot.tgz _bmad/`) MUST pass `observe_only=True` to avoid corrupting their own snapshot. Docstring carries an explicit warning: "Calling this function with the default observe_only=False may write to disk; pass observe_only=True for read-only callers." |
...
| Slug-vs-canonical key reconciliation (gap D10) | Before writing the phase entry, `write_unified_state` calls `sprint_status_get(root, story_key)` to resolve to the canonical `sprint.story` key, and writes under that key. Any orphan slug-keyed entry in the phase store is deleted as part of the same write under lock. This prevents the "two entries for one story" footgun that M48's `compute_dual_state` fallback (`phase_store.get(key) or phase_store.get(sprint.story)`) currently papers over. |
...
- **`read_unified_state(project_root, story_key, *, observe_only=False)` return** (gap D07): if `observe_only=False` (default), returns `tuple[str, str]` — `(sprint_status, phase_value)` (the previous shape; may write to disk on repair/migration). If `observe_only=True`, returns `tuple[str, str, bool]` — `(sprint_status, phase_value, needs_repair)` where `needs_repair=True` flags that the on-disk state is divergent but the reader did NOT mutate it. Forensic / read-only callers MUST pass `observe_only=True`. Tuple-arity-by-flag is acceptable because Python callers explicitly opt-in; if a future caller wants both branches without conditionals, a sibling `read_unified_state_pair(root, key)` returning a dataclass is a follow-up.
- **`write_unified_state(project_root, story_key, sprint_status, phase, *, lock_timeout=10.0)`**: validates that (status, phase) is consistent via `is_consistent(...)`; raises `UnifiedStateError` if not (no silent normalization on write — writers must commit to a coherent pair). `phase` may be passed as `Phase` enum or kebab-case string. **Key reconciliation**: before writing, calls `sprint_status_get(root, story_key)` to resolve to the canonical `sprint.story` id; phase entry is written under that canonical key; any orphan slug-keyed entry in the phase store is deleted under the same lock (gap D10).
...
Minimum **17 new tests** in `tests/test_unified_state.py` (was 12; expanded for gap-report HIGH coverage):
...
15. `test_mtime_tie_on_coarse_filesystem_terminal_phase_wins` (gap D08) — use `os.utime(path, (ts, ts))` to force two writes to the *same* whole-second mtime; one carries a terminal phase, the other a non-terminal; assert terminal wins regardless of write order. **Skip on filesystems that report sub-millisecond granularity if the synthetic tie can't be forced** (use a `mtime_provider` injection or `os.utime` boundary check).
16. `test_read_unified_state_observe_only_no_disk_writes` (gap D07) — legacy single-store fixture (phase store empty); call `read_unified_state(root, key, observe_only=True)`; assert returned 3-tuple has `needs_repair=True`; assert `phase_store_path(root)` is still empty (NO migration); assert sprint-status file bytes byte-identical pre/post.
17. `test_slug_keyed_phase_entry_reconciled_on_write` (gap D10) — fixture where phase store has an entry under descriptive slug `1-1-host-feasibility-probe`; `write_unified_state(root, "1.1", ...)` resolves to canonical `sprint.story` via `sprint_status_get`, writes under that key, deletes the slug-keyed orphan; final phase store has exactly one entry.
...
- **D10** — Writer resolves to canonical `sprint.story` key via `sprint_status_get` before writing; orphan slug-keyed entries are deleted under the same lock. Test #17 verifies reconciliation.
