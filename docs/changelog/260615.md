...
- `scripts/m11-vocabulary-gates.sh` — the frozen ordering signature for Gate 5 (REQ-11) is extended by one line so the new `docs/changelog/260615.md` entry is recognized. No existing entry's line numbers move; the gate's intent (catch movement of historical entries) is preserved.
