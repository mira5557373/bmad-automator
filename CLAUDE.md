...
- **HookBus (N6.2/N6.3)** `core/bauto_bridge/hookbus_shim.py` is the in-process Python callback bus; `commands/orchestrator.py` (NOT `core/gate_orchestrator.py`) fires it at the 6 `KNOWN_EVENTS` lifecycle stages (`post_dev_phase`, `pre_review`, `post_review`, `pre_gate`, `post_gate`, `pre_commit`). Registration order = dispatch order; exceptions in a listener are fail-closed per HookBus contract. `core/gate_orchestrator.py` itself does not import or call HookBus — the bus emits sit on the orchestrator-helper CLI side (see `commands/orchestrator.py` lines 256/259/750/887/892/957).
...
- **Action enum (N6.6)** `core/action_enum.py` — `Literal` type for verifier actions consumed by `route_gate_verdict` and `success_verifiers.production_ready_gate`; closed vocabulary `{"done", "remediate", "park", "defer", "escalate"}` (matches `VALID_ACTIONS` in `core/action_enum.py`).
