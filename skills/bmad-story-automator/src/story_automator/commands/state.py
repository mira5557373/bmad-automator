...
def cmd_build_state_doc(args: list[str]) -> int:
    ...
    if isinstance(agent_config, dict):
        ...
        if "defaultModel" in agent_config:
            lines.append(
                f"  defaultModel: {_yaml_value(_model_or_none(agent_config.get('defaultModel')))}"
            )
        if isinstance(per_task, dict) and per_task:
            ...
            for task in sorted(per_task):
                ...
                if "fallback" in entry:
                    ...
                    lines.append(
                        f"      fallback: {'false' if value is False else _yaml_value(value)}"
                    )
                if "model" in entry:
                    lines.append(
                        f"      model: {_yaml_value(_model_or_none(entry.get('model')))}"
                    )
    ...
    for key, value in replacements.items():
        text = re.sub(
            rf"(?m)^{re.escape(key)}:.*$",
            lambda m, k=key, v=value: f"{k}: {_yaml_value(v)}",
            text,
        )
    story_range = [
        ...
        json.dumps(
            {
                ...
                "reviewCycles": count_matches(
                    metrics_text, r"review cycle|code review cycle"
                ),
